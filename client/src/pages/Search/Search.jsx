    ...
    <div className="min-h-screen bg-[#f8fafc] font-sans text-gray-800 selection:bg-[#076249] selection:text-white">
            ...
            <div className="mb-6">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-3 py-2.5 border border-gray-200 rounded-lg text-sm font-medium focus:outline-none focus:ring-2 focus:ring-[#076249] bg-gray-50"
                />
              </div>
            </div>
        ...
        <div className="flex-1">
          <div className="flex justify-between items-center mb-6">
            <h1 className="text-2xl font-bold text-gray-900">
              {loading ? 'Finding Specialists...' : `${filteredDoctors.length} Specialist${filteredDoctors.length !== 1 ? 's' : ''} found`}
            </h1>
          ...
          <div className="space-y-5">
            ...
            {!loading && filteredDoctors.length === 0 && (
              <div className="bg-white border border-gray-200 rounded-xl p-12 text-center shadow-sm">
                ...
                <h3 className="font-bold text-gray-900 text-lg mb-2">No results found</h3>
                <p className="text-gray-500 text-sm">Try searching with a different name, specialty, or city.</p>
                <button onClick={() => setSearchQuery('')} className="mt-4 text-[#076249] text-sm font-semibold hover:underline">Clear search</button>
              </div>
            )}
                  ...
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center text-sm text-gray-500 gap-3 sm:gap-6 mb-4">
                        <div className="flex items-center"><MapPin size={16} className="mr-1.5 text-gray-400" /><span>{doc.location}</span></div>
                        <div className="flex items-center"><Briefcase size={16} className="mr-1.5 text-gray-400" /><span>{doc.experience}</span></div>
                        {doc.availableNow && (
                          <div className="bg-[#e6f4f0] text-[#076249] px-2.5 py-0.5 rounded-full text-xs font-semibold flex items-center w-max">
                            <span className="w-1.5 h-1.5 bg-[#076249] rounded-full mr-1.5"></span>Available Now
                          </div>
                        )}
                      </div>
                    </div>
          ...
          {!loading && filteredDoctors.length > 0 && (
            <div className="flex justify-center mt-10">
              <div className="flex items-center space-x-1">
                <button className="w-8 h-8 flex items-center justify-center rounded border border-gray-200 text-gray-400 hover:bg-gray-50 transition-colors"><ChevronLeft size={16} /></button>
                ...
                <button className="w-8 h-8 flex items-center justify-center rounded border border-gray-200 text-gray-500 hover:bg-gray-50 transition-colors"><ChevronRight size={16} /></button>
              </div>
            </div>
          )}
        </div>
      ...
      <footer className="mt-16 py-10 px-8 border-t border-gray-200 max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          ...
          <p className="text-gray-500 text-xs max-w-sm leading-relaxed mb-4">Revolutionizing medical logistics and roaming healthcare with high-efficiency provider matching.</p>
